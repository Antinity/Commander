      ...
      async (err, data) => {
        ...
        switch (customId) {
          case "ticket-close":
            ...
            .addFields(
                { name: `${config.ticketTranscriptMember}`, value: `<@${data.OwnerID}>`, inline: true },
                { name: `${config.ticketTranscriptTicket}`, value: `${data.TicketID}`, inline: true },
                { name: `${config.ticketTranscriptTime}`, value: `<t:${transcriptTimestamp}:R>`, inline: true }
            )
            ...
            const owner = await guild.members.cache.get(data.OwnerID);
            await owner
              .send({ embeds: [transcriptEmbed.setDescription(`Your ticket has been closed in **${interaction.guild.name}**.`)], files: [transcript] })
              .catch((error) => {
                return;
              });
